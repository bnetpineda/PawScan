...
const AnalysisModal = ({ visible, isDark, selectedAnalysis, onClose, onShare, onDelete, formatDate }) => {
  if (!selectedAnalysis) return null;

  const text = String(selectedAnalysis.analysis_result ?? "");
      ...
      <View className="flex-1 bg-white dark:bg-black">
        ...
        <ScrollView className="flex-1">
          {selectedAnalysis.image_url ? (
            ...
            <View className="w-full h-96 bg-neutral-200 dark:bg-neutral-800 items-center justify-center">
          ...
          <View className="p-4">
            <View className="flex-row justify-between items-center mb-3">
              <Text className="text-sm font-inter text-neutral-500 dark:text-neutral-400">
                {String(formatDate(selectedAnalysis.created_at))}
              </Text>
              <View className={`self-start px-2 py-1 rounded-full ${urgencyColor}`}>
                <Text className="text-xs font-inter-semibold text-white">{urgencyLevel}</Text>
              </View>
            </View>

            <Text className="text-base font-inter text-black dark:text-white mb-6">{text}</Text>

            <View className="flex-row justify-around border-t border-neutral-200 dark:border-neutral-800 pt-3">
              <TouchableOpacity
                className="flex-row items-center"
                onPress={() => onShare(selectedAnalysis)}
              >
              ...
              <TouchableOpacity
                className="flex-row items-center"
                onPress={() => onDelete(selectedAnalysis.id)}
              >
