...
const VetsListScreen = () => {
  ...
  const renderVet = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => router.push(`/(vet)/chat/${item.id}?userName=${encodeURIComponent(item.name)}`)}
      activeOpacity={0.7}
    >
      <View className="mr-4">
  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-black pt-12">
      ...
      <View className="px-5 py-4 border-b border-black dark:border-neutral-700">
        <Text className="text-2xl font-inter-bold text-black dark:text-white">Select a User</Text>
      </View>
      {loading ? (
        <View className="flex-1 justify-center items-center p-8">
          <ActivityIndicator size="large" color={isDark ? "#fff" : "#000"} />
          <Text className="mt-4 text-base text-gray-600 dark:text-gray-300">Loading users...</Text>
        </View>
      ) : vets.length === 0 ? (
        <View className="flex-1 justify-center items-center p-8">
          <FontAwesome name="user" size={64} color={isDark ? "#fff" : "#000"} />
          <Text className="text-2xl font-inter-bold mt-4 mb-2 text-black dark:text-white">No users available</Text>
